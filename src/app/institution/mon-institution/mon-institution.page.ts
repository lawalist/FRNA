...
@Component({
  selector: 'app-mon-institution',
  templateUrl: './mon-institution.page.html',
  styleUrls: ['./mon-institution.page.scss'],
})
export class MonInstitutionPage implements OnInit {
  ...
  editForm(pDoc){
    ...
    this.partenaireForm = this.formBuilder.group({
      monInstitution: [p.monInstitution],
  ...
  async presentLocalite(idLocalite) {
        ...
        buttons: [
          {
            ...
            handler: () => {
              for(let numero of numeros){
                ...
                this.servicePouchdb.findRelationalDocByID('partenaire', numero).then((res) => {
    ...
    cocherElemListe(numero){
      if(this.selectedIndexes.indexOf(numero) === -1){
        //si coché
        this.selectedIndexes.push(numero);
      }else{
        //si décocher
        this.selectedIndexes.splice(this.selectedIndexes.indexOf(numero), 1);
      }  
    ...
    cocherTousElemListe(){
      this.partenairesData.forEach((p) => {
        ...
        if(this.selectedIndexes.indexOf(p.numero) === -1){
          this.selectedIndexes.push(p.numero);
        }
      });
   ...
   getPartenaireWithConflicts(event = null){
    ...
    getPartenaire(){
      ...
      if(this.idPartenaire && this.idPartenaire != ''){
        ...
        let archived = {$ne: null};
        let deleted = false;
        let shared = {$ne: null};
        this.servicePouchdb.getMonInstitution('partenaire', true, deleted, archived, shared).then((res) => {
          if(res && res.partenaires && res.partenaires.length > 0){

            for(let p of res.partenaires){
              if(p.formData.monInstitution){
                let idPays;
                let idRegion;
                let idDepartement;
                let idCommune;
                ...
                //partenairesData.push({...p.formData, ...p.formioData, ...p.security});
                this.infos({id: p.id, idPays: idPays, idRegion: idRegion, idDepartement: idDepartement, idCommune: idCommune, idSiege: idSiege, ...p.formData, ...p.formioData, ...p.security}); 
                break;
              }//end if
          ...
          this.action = 'liste';
          console.log('Erreur acces aux mon institution');
          console.log(err);
        });
      }
