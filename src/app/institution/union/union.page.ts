...
export class UnionPage implements OnInit {
  ...
  colonnes = ['nom', 'numero', 'niveau', 'nomFederation', 'numeroFederation', 'dateCreation', 'codePays', 'nomPays', 'codeRegion', 'nomRegion', 'codeDepartement', 'nomDepartement', 'codeCommune', 'nomCommune', 'codeSiege', 'nomSiege', 'latitude', 'longitude']
  ...
  addItemToObjectAtSpecificPosition (obj, key, value, index) {
    ...
    getUnionWithConflicts(event = null){
      ...
      this.servicePouchdb.findRelationalDocInConflict('union').then((res) => {
        if(res){
          ...
          let communeIndex = [];
          let localiteIndex = [];
          let idFederation, idPays, idRegion, idDepartement, idCommune, idLocalite;
          ...
          for(let u of res.unions){
            ...
            if(isDefined(localiteIndex[u.localite])){
              ...
              idLocalite = res.localites[localiteIndex[u.localite]].id;
            }else{
            ...
            unionsData.push({id: u.id, idFederation: idFederation, idPays: idPays, idRegion: idRegion, idDepartement: idDepartement, idCommune: idCommune, idLocalite: idLocalite, ...u.formData, ...u.formioData, ...u.security});
          }
    ...
    async presentDerniereModification(union) {
      const modal = await this.modalController.create({
        ...
        componentProps: { _id: union.id, _rev: union.rev, security: union.security },
        mode: 'ios',
        //cssClass: 'costom-modal',
      });
      return await modal.present();
    ...
    async openRelationUnion(ev: any/*, numeroUnion*/) {
      const popover = await this.popoverController.create({
        ...
        event: ev,
        translucent: true,
        componentProps: {"idUnion": this.uneUnion.id},
        animated: true,
        showBackdrop: true,
        //mode: "ios"
      });
    ...
    async openRelationUnionDepuisListe(ev: any/*, codePays*/) {
      const popover = await this.popoverController.create({
        ...
        event: ev,
        translucent: true,
        componentProps: {"idUnion": this.selectedIndexes[0]},
        animated: true,
        showBackdrop: true,
        //mode: "ios"
      });
    ...
    async openRelationUnionDepuisTableau(ev: any/*, codePays*/) {
      ...
      const popover = await this.popoverController.create({
        ...
        event: ev,
        translucent: true,
        componentProps: {"idUnion": this.selectedIndexes[0]/*data.numero*/},
        animated: true,
        showBackdrop: true,
        mode: "ios"
      });
    ...
    onSubmit(){
      ...
      if(this.action === 'ajouter'){
        ...
        this.servicePouchdb.createRelationalDoc(doc).then((res) => {
          ...
          let unionData = {id: res.id, ...union.formData, ...union.formioData, ...union.security};
          //this.unions = union;
      ...
      if(this.idUnion && this.idUnion != ''){
        this.servicePouchdb.findRelationalDocByID('union', this.idUnion).then((res) => {
          if(res && res.unions[0]){
            ...
            this.infos({id: res.unions[0].id, idFederation: f, idPays: res.pays[0].id, idRegion: res.regions[0].id, idDepartement: res.departements[0].id, idCommune: res.communes[0].id, idLocalite: res.localites[0].id, ...res.unions[0].formData}); 
          }else{
            alert(this.translate.instant('GENERAL.ENREGISTREMENT_NOT_FOUND'));
            this.close();
          }
          ...
          if(res && res.unions){
            ...
            if(this.mobile){
              ...
              $('#union-partenaire').ready(()=>{
                if(global.langue == 'en'){
                  this.unionHTMLTable = createDataTable("union-partenaire", this.colonnes, unionsData, null, this.translate, global.peutExporterDonnees);
                }else{
                  this.unionHTMLTable = createDataTable("union-partenaire", this.colonnes, unionsData, global.dataTable_fr, this.translate, global.peutExporterDonnees);
                }
                this.attacheEventToDataTable(this.unionHTMLTable.datatable);
              });
            }
